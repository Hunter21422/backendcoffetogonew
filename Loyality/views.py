...
class TelegramAuthView(APIView):
    ...
    def post(self, request):
        ...
        valid, result = validate_telegram_init_data(init_data, settings.TELEGRAM_BOT_TOKEN)
        if not valid:
        ...
        try:
            with transaction.atomic():
                user, created = User.objects.get_or_create(
                    telegram_id=tg_id,
                    defaults={
                        'username': tg_username or f"tg_{tg_id}",
                ...
                if created:
                    LoyaltyProfile.objects.create(user=user)
        except IntegrityError:
            return Response({"detail": "Ошибка создания пользователя"}, status=status.HTTP_400_BAD_REQUEST)
...
def add_stamp_by_telegram(request):
    ...
    if not telegram_username:
        return Response({"detail": "telegram_username required"}, status=400)
    ...
    profile, _ = LoyaltyProfile.objects.get_or_create(user=user)
    max_stamps = getattr(settings, "LOYALTY_MAX_STAMPS", 6)
    old_stamps = profile.stamps
    new_stamps = min(old_stamps + amount, max_stamps)
    added = new_stamps - old_stamps

    if added <= 0:
        return Response({"detail": f"Лимит достигнут ({max_stamps})"}, status=400)

    profile.stamps = new_stamps
    profile.save(update_fields=["stamps"])

    for _ in range(added):
        LoyaltyStamp.objects.create(
            user=user,
            source="manual_barista",
            created_by=request.user
        )

    return Response({
        "detail": f"Начислено {added} штамп(ов)",
        "stamps_added": added,
        "stamps_total": new_stamps,
        "max_stamps": max_stamps,
        "at_limit": new_stamps >= max_stamps
    })
